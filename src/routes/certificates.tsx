      ...
      <main className="mx-auto max-w-5xl space-y-12">
        ...
        <div className="bg-white border border-border rounded-2xl p-8 shadow-sm">
          ...
          <p className="text-xs text-text-muted mb-6 max-w-xl">
            Employers or system regulators can run immediate token compliance lookups on credential
            hashes below.
          </p>
          ...
          <form onSubmit={handleVerifyLookup} className="flex gap-4 max-w-2xl">
            ...
            <button
              type="submit"
              className="bg-brand-primary text-white rounded-xl px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-brand-dark transition-all"
            >
              Verify
            </button>
          </form>
        ...
        <div className="space-y-4">
          {mockupCertificates.map((cert) => (
            <div
              key={cert.id}
              className={`border rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all ${cert.isLocked ? "bg-bg-muted border-border" : "bg-white border-border shadow-sm"}`}
            >
              <div className="flex items-start gap-4">
                <div
                  className={`p-3 rounded-xl ${cert.isLocked ? "bg-border text-text-muted" : "bg-brand-primary/10 text-brand-primary"}`}
                >
                ...
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span
                      className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${cert.isLocked ? "bg-border text-text-muted" : "bg-accent-green/10 text-accent-green"}`}
                    >
                      {cert.status}
                    </span>
                  ...
                  <h3
                    className={`text-lg font-bold ${cert.isLocked ? "text-text-muted" : "text-text-primary"}`}
                  >
                    {cert.title}
                  </h3>
                  {cert.isLocked ? (
                    <p className="text-xs font-bold text-brand-primary mt-1 italic">
                      {cert.requirement}
                    </p>
                  ) : (
                    <div className="flex gap-4 mt-1 text-[10px] font-bold text-text-muted uppercase tracking-widest">
                      <span className="flex items-center gap-1.5">
                        <Calendar className="h-3.5 w-3.5" /> {cert.issueDate}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <ShieldCheck className="h-3.5 w-3.5" /> {cert.credentialId}
                      </span>
                    </div>
                  )}
                </div>
